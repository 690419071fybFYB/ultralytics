#!/usr/bin/env python3
"""Train CenterRTDETRDecoder with configurable fusion strategy and center lambda."""
...
def parse_args() -> argparse.Namespace:
    """Parse CLI args for center decoder ablations."""
    parser = argparse.ArgumentParser(description="Train CenterRTDETRDecoder with ablation controls.")
    parser.add_argument(
        "--model",
        type=str,
        default="ultralytics/cfg/models/rt-detr/rtdetr-l-center.yaml",
    ...
    parser.add_argument("--fraction", type=float, default=0.01, help="Fraction of training set to use.")
...
def default_run_name(args: argparse.Namespace) -> str:
    """Generate a compact run name for ablation tracking."""
    fusion = args.center_fusion_strategy
    lam = str(args.center_lambda_max).replace(".", "")
    return f"center_{fusion}_lam{lam}_seed{args.seed}"


def main() -> None:
    """Launch center decoder training with configurable fusion and lambda."""
    args = parse_args()
